...
class GenerateFreqAttribute(GenerateAttribute):
  ...
  def __init__(self, **kwargs):
    ...
    for rec_list in freq_file_data:
      if (len(rec_list) != 2):
          raise Exception, 'Illegal format in frequency file %s: %s' %(self.freq_file_name, line)
          #raise Exception, 'Illegal format in frequency file %s: %s' %(self.freq_file_name, line)
...
class GenerateMyCompoundAttribute(GenerateCompoundAttribute):
  ...
  def __init__(self, **kwargs):
    ...
    for (keyword, value) in kwargs.items():
      ...
      if (keyword.startswith('function1')):
        basefunctions.check_is_function_or_method('function1', value)
        self.function1 = value
      elif (keyword.startswith('funcition2')):
        basefunctions.check_is_function_or_method('function2', value)
        self.function2 = value
      elif (keyword.startswith('parameters_funcition1')):
        basefunctions.check_is_list('parameters_funcition1', value)
        if (len(value) > 5):
          raise Exception, 'Maximum five parameters allowed for function call'
        self.parameters1 = value
      elif (keyword.startswith('parameters_funcition2')):
        basefunctions.check_is_list('parameters_funcition2', value)
        if (len(value) > 5):
          raise Exception, 'Maximum five parameters allowed for function call'
        self.parameters2 = value
      else:
        base_kwargs[keyword] = value
    ...
    # Check if the necessary variables have been set
    #
    basefunctions.check_is_function_or_method('function1', self.function1)
    basefunctions.check_is_function_or_method('function2', self.function2)

    # Check if the function does return a string (five different possibilities,
    # depending upon number of parameters)
    #
    if (self.parameters1 == None) or (len(self.parameters1) == 0):
      funct_ret = self.function1()
    elif (len(self.parameters1) == 1):
      funct_ret = self.function1(self.parameters1[0])
    elif (len(self.parameters1) == 2):
      funct_ret = self.function1(self.parameters1[0], self.parameters1[1])
    elif (len(self.parameters1) == 3):
      funct_ret = self.function1(self.parameters1[0], self.parameters1[1],
                                self.parameters1[2])
    elif (len(self.parameters1) == 4):
      funct_ret = self.function1(self.parameters1[0], self.parameters1[1],
                                self.parameters1[2], self.parameters1[3])
    else:
      funct_ret = self.function1(self.parameters1[0], self.parameters1[1],
                                self.parameters1[2], self.parameters1[3],
                                self.parameters1[4])
  ...
  def create_attribute_value(self):
    ...
    if (self.parameters1 == None):
      funct_ret1 = self.function1()
    elif (len(self.parameters1) == 1):
      funct_ret1 = self.function1(self.parameters1[0])
    elif (len(self.parameters1) == 2):
      funct_ret1 = self.function1(self.parameters1[0], self.parameters1[1])
    elif (len(self.parameters1) == 3):
      funct_ret1 = self.function1(self.parameters1[0], self.parameters1[1],
                                self.parameters1[2])
    elif (len(self.parameters1) == 4):
      ...
      funct_ret1 = self.function1(self.parameters1[0], self.parameters1[1],
                                self.parameters1[2], self.parameters1[3],
                                self.parameters[4])
    if (self.parameters2 == None):
      funct_ret2 = self.function2()
    elif (len(self.parameters2) == 1):
      funct_ret2 = self.function2(self.parameters2[0])
    elif (len(self.parameters2) == 2):
      funct_ret2 = self.function2(self.parameters2[0], self.parameters2[1])
    elif (len(self.parameters2) == 3):
      funct_ret2 = self.function2(self.parameters2[0], self.parameters2[1],
                                self.parameters2[2])
    elif (len(self.parameters2) == 4):
      funct_ret2 = self.function2(self.parameters2[0], self.parameters2[1],
                                self.parameters2[2], self.parameters2[3])
    else:
      funct_ret2 = self.function2(self.parameters2[0], self.parameters2[1],
                                self.parameters2[2], self.parameters2[3],
                                self.parameters[4])
      
    return funct_ret1 , funct_ret2

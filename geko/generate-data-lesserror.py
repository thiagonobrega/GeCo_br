...
#
out_file_name = 'data/data-20-5.csv'
...
#thiago
birth_date_uniform_attr = \
     generator.GenerateFuncAttribute(attribute_name = 'birth',
                        function = attrgenfunct.generate_normal_date,
                        parameters = [1,12,1904,2000])

death_date_uniform_attr = \
     generator.GenerateFuncAttribute(attribute_name = 'death',
                        function = attrgenfunct.generate_normal_date,
                        parameters = [1,12,2001,2016])
     
income_normal_attr = \
    generator.GenerateFuncAttribute(attribute_name = 'income-normal',
                       function = attrgenfunct.generate_normal_value,
                       parameters = [1200,500, 0, 4500, 'float2'])
...
#thiago
# age_birth_comp_attr = \
#     generator.GenerateMyCompoundAttribute(\
#           continuous1_attribute_name = 'ageb',
#           continuous2_attribute_name = 'birth',
#           continuous1_funct_name =     'uniform',
#           continuous1_funct_param =    [10,110],
#           continuous1_value_type = 'int',
#           continuous2_value_type = 'int',
#           continuous2_function = contdepfunct.birth_depending_on_age )
...
ocr_corruptor_date = corruptor.CorruptValueOCR(\
          position_function = corruptor.position_mod_normal,
          lookup_file_name = 'lookup-files/date-variations.csv',
          has_header_line = False,
          unicode_encoding = unicode_encoding_used)
...
#thiago
my_val_corruptor = corruptor2.CorruptIncomeValue()
my_date_corruptor = corruptor2.CorruptDateValue()
...
# itself.
#
attr_name_list = ['given-name', 'surname',
                  'birth', 'death',
                   'income-normal']


attr_data_list = [gname_attr, sname_attr,
                  income_normal_attr, birth_date_uniform_attr , 
                  death_date_uniform_attr]
...
#
attr_mod_prob_dictionary = {'death':0.15, 'given-name':0.15,'surname':0.15,
                            'income-normal':0.35,
                            'birth':0.20}
...
#
attr_mod_data_dictionary = {'death':[(0.25, ocr_corruptor_date),
                                     (0.75, my_date_corruptor)],
                            'surname':[(0.2, surname_misspell_corruptor),
                                       (0.2, ocr_corruptor),
                                       (0.1, keyboard_corruptor),
                                       (0.1, missing_val_corruptor),
                                       (0.4, phonetic_corruptor)],
                            'given-name':[(0.2, edit_corruptor2),
                                          (0.2, ocr_corruptor),
                                          (0.1, keyboard_corruptor),
                                          (0.1, missing_val_corruptor),
                                          (0.4, phonetic_corruptor)],
                            'income-normal':[(1,my_val_corruptor)],
                            'birth':[(0.25, ocr_corruptor_date),
                                     (0.75, my_date_corruptor)]}

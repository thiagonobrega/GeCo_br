...
def age_depending_on_birth(birthday):
  """
  Generate age based on date (BR DATE) DAY/MONTH/YEAR [DD/MM/YYYY]   
  """
  now = datetime.now().time()
  bd = datetime.strptime(birthday, '%d/%m/%Y')  

  return int( (now - bd)/365 )
  

def birth_depending_on_age(age):
  """     
  """
  now = datetime.now()
  year = now.year - age
  ...
  import attrgenfunct
  bd = attrgenfunct.generate_normal_date(1,12,year,year)    
